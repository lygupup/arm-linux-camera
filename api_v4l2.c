...
int linux_v4l2_device_init(const char *dev)//设置视频格式，
{
	struct v4l2_capability cap; //获取摄像设备支持
...
//--------------------------------------------------------------------------------------	
    ...
    fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;//格式为预览
...
/**************************获取或者设置针频率******************************************/
    ...
    memset(setfps, 0, sizeof(struct v4l2_streamparm));//初始化
	
    setfps->type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    ret = ioctl(fd, VIDIOC_G_PARM, setfps);//获取针频率参数
    if(ret == 0)
    {
        printf("\n  Frame rate:   %u/%u\n",
    ...
    if (setfps->parm.capture.timeperframe.numerator == 1
	        && setfps->parm.capture.timeperframe.denominator == FPS)//FPS=30
    {
    ...
    else
    {
        ...
        setfps->parm.capture.timeperframe.denominator=FPS; //采集频率设置为1秒钟30针
        ret = ioctl(fd, VIDIOC_S_PARM, setfps);//设置视频采集频率
        if(ret == -1)
        {
            perror("Unable to set frame rate");
            return ret;
        }
		ret = ioctl(fd, VIDIOC_G_PARM, setfps);//或去视频采集频率
        if(ret == 0)
        {
...
int linux_v4l2_init_mmap()//创建四个缓冲区
{
    ...
    // 请求分配内存
   	printf("*************************\nrequesbuffers\n");
	reqbuf.count = BUFFER_COUNT;//设置申请缓冲区个数 4个  （内核空间4个缓冲区）
	reqbuf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;//视频输出预览
	...
	{
	    memset(&buf, 0, sizeof(struct v4l2_buffer));//初始化
		
	    buf.index = i;//指定为第i个缓冲区
